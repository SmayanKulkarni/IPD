import argparse, os, yaml, numpy as np, json
...
def evaluate(pipeline_type):
    ...
    # Fix: Ensure dvclive directory exists
    ...
    plt.savefig(f"dvclive/{pipeline_type}_confusion_matrix.png")
    plt.close()

    print(f"{pipeline_type.upper()} | Acc: {acc:.4f} | KSI: {avg_ksi:.4f}")
